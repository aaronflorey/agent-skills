Source: https://fishshell.com/docs/current/cmds/status.html

# status - query fish runtime information[¶](#status-query-fish-runtime-information "Link to this heading")
...
status line-number
status stack-trace
status job-control CONTROL\_TYPE
status features
...
Returns 0 if fish is currently showing a prompt in the context of a [breakpoint](breakpoint.html) command. See also the [fish\_breakpoint\_prompt](fish_breakpoint_prompt.html) function.
...
Returns 0 if fish is running an interactive [read](read.html) builtin which is connected to a keyboard.

**is-login**, **\-l** or **\--is-login**

Returns 0 if fish is a login shell - that is, if fish should perform login tasks such as setting up [`PATH`](../language.html#envvar-PATH).
...
Prints the name of the currently-running function or command, like the deprecated [`_`](../language.html#envvar-_) variable.
...
Prints the filename of the currently-running script. If the current script was called via a symlink, this will return the symlink. If the current script was received by piping into [source](source.html), then this will return `-`.
...
Prints just the path to the running script, without the actual filename itself. This can be relative to [`PWD`](../language.html#envvar-PWD) (including just “.”), depending on how the script was called. This is the same as passing the filename to `dirname(3)`. It’s useful if you want to use other files in the current script’s directory or similar.
...
**job-control**, **\-j** or **\--job-control** _CONTROL\_TYPE_

Sets the job control type to _CONTROL\_TYPE_, which can be **none**, **full**, or **interactive**.

**features**

Lists all available [feature flags](../language.html#featureflags).

**test-feature** _FEATURE_
...
This lists the files embedded in the fish binary at compile time. Only files where the path starts with the optional _FILE_ argument are shown. Returns 0 if something was printed, 1 otherwise.

**terminal**

Prints the name and version of the terminal fish is running inside (for example as reported via [XTVERSION](../terminal-compatibility.html#term-compat-xtversion)). This is not available during early startup but only starting from when the first interactive prompt is shown, possibly via builtin [read](read.html), so before the first `fish_prompt` or `fish_read` [event](../language.html#event).

**terminal-os**

Prints the name of the operating system (OS) the terminal is running on, as reported via [XTGETTCAP query-os-name](../terminal-compatibility.html#term-compat-xtgettcap). Like [status terminal](#status-terminal), this only works once the first interactive prompt is shown. Returns 1 if the OS name is not available.

**test-terminal-feature** _FEATURE_

Returns 0 when the terminal was [detected](../terminal-compatibility.html#term-compat-xtgettcap) to support the given feature. Like [status terminal](#status-terminal), this only works once the first interactive prompt is shown.

Currently the only available _FEATURE_ is [scroll-content-up](../terminal-compatibility.html#term-compat-indn). An error will be printed when passed an unrecognized feature.
