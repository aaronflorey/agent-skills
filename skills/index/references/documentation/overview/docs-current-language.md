Source: https://fishshell.com/docs/current/language.html

# The fish language[¶](#the-fish-language "Link to this heading")

This document is a comprehensive overview of fish’s scripting language.

For interactive features see [Interactive use](interactive.html).
...
[echo](cmds/echo.html) command writes its arguments to the screen. In this example the output is `hello world`.
...
-   [cd](cmds/cd.html): Change the current directory
...
-   [open](cmds/open.html): Open files with the default application associated with each filetype
...
## Quotes[¶](#quotes "Link to this heading")
...
## Escaping Characters[¶](#escaping-characters "Link to this heading")
...
## Input/Output Redirection[¶](#input-output-redirection "Link to this heading")
...
The destination of a stream can be changed using something called _redirection_. For example, `echo hello > output.txt`, redirects the standard output of the `echo` command to a text file.
...
-   To write standard error to a file, use `2>DESTINATION`. [\[1\]](#id3)
...
## Piping[¶](#piping "Link to this heading")

Another way to redirect streams is a _pipe_. A pipe connects streams with each other. Usually the standard output of one command is connected with the standard input of another. This is done by separating commands with the pipe character `|`. For example:
...
will attempt to build `fish`, and any errors will be shown using the `less` pager. [\[2\]](#id5)
...
## Combining pipes and redirections[¶](#combining-pipes-and-redirections "Link to this heading")
...
This is important when any redirections reference other file descriptors with the `&N` syntax. When you say `>&2`, that will redirect stdout to where stderr is pointing to _at that time_.
...
## Job control[¶](#job-control "Link to this heading")
...
will start the emacs text editor in the background. [fg](cmds/fg.html) can be used to bring it into the foreground again when needed.

Most programs allow you to suspend the program’s execution and return control to fish by pressing ctrl\-z (also referred to as `^Z`). Once back at the fish commandline, you can start other programs and do anything you want. If you then want you can go back to the suspended command by using the [fg](cmds/fg.html) (foreground) command.

If you instead want to put a suspended job into the background, use the [bg](cmds/bg.html) command.

To get a listing of all currently started jobs, use the [jobs](cmds/jobs.html) command. These listed jobs can be removed with the [disown](cmds/disown.html) command.

At the moment, functions cannot be started in the background. Functions that are stopped and then restarted in the background using the [bg](cmds/bg.html) command will not execute correctly.
...
Fish’s prompt is also defined in a function, called [fish\_prompt](cmds/fish_prompt.html). It is run when the prompt is about to be displayed and its output forms the prompt:
...
function fish\_prompt
    ...
    echo (set\_color yellow)$PWD (set\_color purple)$user\_char
end
COPY

To edit a function, you can use [funced](cmds/funced.html), and to save a function [funcsave](cmds/funcsave.html). This will store it in a function file that fish will [autoload](#syntax-function-autoloading) when needed.

The [functions](cmds/functions.html) builtin can show a function’s current definition (and [type](cmds/type.html) will also do if given a function).

For more information on functions, see the documentation for the [function](cmds/function.html) builtin.

### Defining aliases[¶](#defining-aliases "Link to this heading")
...
To easily create a function of this form, you can use the [alias](cmds/alias.html) command. Unlike other shells, this just makes functions - fish has no separate concept of an “alias”, we just use the word for a simple wrapping function like this. [alias](cmds/alias.html) immediately creates a function. Consider using `alias --save` or [funcsave](cmds/funcsave.html) to save the created function into an autoload file instead of recreating the alias each time.

For an alternative, try [abbreviations](interactive.html#abbreviations). These are words that are expanded while you type, instead of being actual functions inside the shell.
...
For example if you try to execute something called `banana`, fish will go through all directories in $fish\_function\_path looking for a file called `banana.fish` and load the first one it finds.
...
As we’ve explained, autoload files are loaded _by name_, so, while you can put multiple functions into one file, the file will only be loaded automatically once you try to execute the one that shares the name.
...
If a file of the right name doesn’t define the function, fish will not read other autoload files, instead it will go on to try builtins and finally commands. This allows masking a function defined later in $fish\_function\_path, e.g. if your administrator has put something into /etc/fish/functions that you want to skip.
...
Fish has some builtins that let you execute commands only if a specific criterion is met: [if](cmds/if.html), [switch](cmds/switch.html), [and](cmds/and.html) and [or](cmds/or.html), and also the familiar [&&/||](#syntax-combiners) syntax.

### The `if` statement[¶](#the-if-statement "Link to this heading")

The [if](cmds/if.html) statement runs a block of commands if the condition was true.

Like other shells, but unlike typical programming languages you might know, the condition here is a _command_. Fish runs it, and if it returns a true [exit status](#variables-status) (that’s 0), the if-block is run. For example:
...
This uses the [test](cmds/test.html) command to see if the file /etc/os-release exists. If it does, it runs `cat`, which prints it on the screen.
...
A more complicated example with a [command substitution](#expand-command-substitution):
...
`if` can also take `else if` clauses with additional conditions and an [else](cmds/else.html) clause that is executed when everything else was false:
...
The [not](cmds/not.html) keyword can be used to invert the status:
...
-   [contains](cmds/contains.html) - to see if a list contains a specific element (`if contains -- /usr/bin $PATH`)
    
-   [string](cmds/string.html) - to e.g. match strings (`if string match -q -- '*-' $arg`)
    
-   [path](cmds/path.html) - to check if paths of some criteria exist (`if path is -rf -- ~/.config/fish/config.fish`)
    
-   [type](cmds/type.html) - to see if a command, function or builtin exists (`if type -q git`)
    

### The `switch` statement[¶](#the-switch-statement "Link to this heading")

The [switch](cmds/switch.html) command is used to execute one of possibly many blocks of commands depending on the value of a string. It can take multiple [case](cmds/case.html) blocks that are executed when the string matches. They can take [wildcards](#expand-wildcard). For example:
...
### Combiners (`and` / `or` / `&&` / `||`)[¶](#combiners-and-or "Link to this heading")

For simple checks, you can use combiners. [and](cmds/and.html) or `&&` run the second command if the first succeeded, while [or](cmds/or.html) or `||` run it if the first failed. For example:

\# $XDG\_CONFIG\_HOME is a standard place to store configuration.
\# If it's not set applications should use ~/.config.
set \-q XDG\_CONFIG\_HOME; and set \-l configdir $XDG\_CONFIG\_HOME
or set \-l configdir ~/.config
COPY

Note that combiners are _lazy_ - only the part that is necessary to determine the final status is run.
...
## Loops and blocks[¶](#loops-and-blocks "Link to this heading")

Like most programming language, fish also has the familiar [while](cmds/while.html) and [for](cmds/for.html) loops.

`while` works like a repeated [if](#syntax-if):
...
will print each file in the current directory. The part after the `in` is a list of arguments, so you can use any [expansions](#expand) there:
...
[break](cmds/break.html) is available to break out of a loop, and [continue](cmds/continue.html) to jump to the next iteration.
...
In addition there’s a [begin](cmds/begin.html) block that just groups commands together so you can redirect to a block or use a new [variable scope](#variables-scope) without any repetition:
...
## Parameter expansion[¶](#parameter-expansion "Link to this heading")
...
### Wildcards (“Globbing”)[¶](#wildcards-globbing "Link to this heading")
...
-   `?` can match any single character except `/`. This is deprecated and can be disabled via the `qmark-noglob` [feature flag](#featureflags), so `?` will be an ordinary character.
...
### Variable expansion[¶](#variable-expansion "Link to this heading")
...
which will replace `$HOME` with the home directory of the current user, and pass it to [echo](cmds/echo.html), which will then print it.

Some variables like `$HOME` are already set because fish sets them by default or because fish’s parent process passed them to fish when it started it. You can define your own variables by setting them with [set](cmds/set.html):

set my\_directory /home/cooluser/mystuff
ls $my\_directory
...
#### Quoting variables[¶](#quoting-variables "Link to this heading")
...
Fish variables are all [lists](#variables-lists), and they are split into elements when they are _set_ - that means it is important to decide whether to use quotes or not with [set](cmds/set.html):
...
This is unlike other shells, which do what is known as “Word Splitting”, where they split the variable when it is _used_ in an expansion. E.g. in bash:
...
If a variable expands to nothing, it will cancel out any other strings attached to it. See the [Combining Lists](#cartesian-product) section for more information.

Most of the time, not quoting a variable is correct. The exception is when you need to ensure that the variable is passed as one element, even if it might be unset or have multiple elements. This happens often with [test](cmds/test.html):
...
#### Dereferencing variables[¶](#dereferencing-variables "Link to this heading")
...
Of course the variable will have to be accessible from the function, so it needs to be [global/universal](#variables-scope) or [exported](#variables-export). It also can’t clash with a variable name used inside the function. So if we had made $foo there a local variable, or if we had named it “arg” instead, it would not have worked.

When using this feature together with [slices](#expand-slices), the slices will be used from the inside out. `$$foo[5]` will use the fifth element of `$foo` as a variable name, instead of giving the fifth element of all the variables $foo refers to. That would instead be expressed as `$$foo[1..-1][5]` (take all elements of `$foo`, use them as variable names, then give the fifth element of those).
...
#### Variables as command[¶](#variables-as-command "Link to this heading")
...
For instance `set if $if` won’t allow you to make an if-block, and `set cmd command` won’t allow you to use the [command](cmds/command.html) decorator, but only uses like `$cmd -q foo`.

### Command substitution[¶](#command-substitution "Link to this heading")

A `command substitution` is an expansion that uses the _output_ of a command as the arguments to another. For example:

echo $(pwd)
COPY

This executes the [pwd](cmds/pwd.html) command, takes its output (more specifically what it wrote to the standard output “stdout” stream) and uses it as arguments to [echo](cmds/echo.html). So the inner command (the `pwd`) is run first and has to complete before the outer command can even be started.
...
If the output is piped to [string split or string split0](cmds/string-split.html) as the last step, those splits are used as they appear instead of splitting lines.
...
The exit status of the last run command substitution is available in the [status](#variables-status) variable if the substitution happens in the context of a [set](cmds/set.html) command (so `if set -l (something)` checks if `something` returned true).
...
but if you need multiple or the command doesn’t read from standard input, “process substitution” is useful. Other shells allow this via `foo <(bar) <(baz)`, and fish uses the [psub](cmds/psub.html) command:
...
If after expansion there is nothing between the braces, the argument will be removed (see [the Combining Lists](#cartesian-product) section):
...
The very first character of a command token is never interpreted as expanding brace, because it’s the beginning of a [compound statement](cmds/begin.html):
...
### Combining lists[¶](#combining-lists "Link to this heading")

Fish expands lists like [brace expansions](#expand-brace):
...
This also happens after [command substitution](#expand-command-substitution). To avoid tokens disappearing there, make the inner command return a trailing newline, or double-quote it:
...
### Slices[¶](#slices "Link to this heading")
...
### Home directory expansion[¶](#home-directory-expansion "Link to this heading")
...
### Combining different expansions[¶](#combining-different-expansions "Link to this heading")
...
## Table Of Operators[¶](#table-of-operators "Link to this heading")
...
[Variable expansion](#expand-variable)
...
[Command substitution](#expand-command-substitution)
...
[Redirection](#redirects), like `command > file`
...
[Pipe](#pipes), connect two or more commands
...
[Backgrounding](#syntax-job-control)
...
[Home directory expansion](#expand-home)
...
## Shell variables[¶](#shell-variables "Link to this heading")
...
To set a variable value, use the [set](cmds/set.html) command. A variable name can not be empty and can contain only letters, digits, and underscores. It may begin and end with any of those characters.
...
set smurf\_color blue
echo Smurfs are usually $smurf\_color
set pants\_color red
echo Papa smurf, who is $smurf\_color, wears $pants\_color pants
COPY

So you set a variable with `set`, and use it with a `$` and the name.

### Variable Scope[¶](#variable-scope "Link to this heading")
...
set \-g fish\_prompt\_pwd\_dir\_length 0

\# Set my preferred cursor style:
function setcursors
   set \-g fish\_cursor\_default block
   set \-g fish\_cursor\_insert line
   set \-g fish\_cursor\_visual underscore
end

\# Set my language
set \-gx LANG de\_DE.UTF-8
COPY
...
### Overriding variables for a single command[¶](#overriding-variables-for-a-single-command "Link to this heading")
...
Unlike other shells, this does _not_ inhibit any lookup (aliases or similar). Calling a command after setting a variable override will result in the exact same command being run.

This syntax is supported since fish 3.1.

### Universal Variables[¶](#universal-variables "Link to this heading")

Universal variables are variables that are shared between all the user’s fish sessions on the computer. All changes to universal variables are persistent and instantly propagated across fish sessions.
...
Do not append to universal variables in [config.fish](#configuration), because these variables will then get longer with each new shell instance. Instead, set them once at the command line.

### Exporting variables[¶](#exporting-variables "Link to this heading")
...
If you specify a negative index when expanding or assigning to a list variable, the index will be taken from the _end_ of the list. For example, the index -1 is the last element of the list:
...
\> set \-x smurf\_PATH forest mushroom
\> env | grep smurf
smurf=blue small
smurf\_PATH=forest:mushroom
COPY
...
Lists can be inspected with the [count](cmds/count.html) or the [contains](cmds/contains.html) commands:
...
set \-l grep\_args \-r "my string"
grep $grep\_args . \# will run the same as \`grep -r "my string"\` .
COPY
...
### Argument Handling[¶](#argument-handling "Link to this heading")
...
A more robust approach to option handling is [argparse](cmds/argparse.html), which checks the defined options and puts them into various variables, leaving only the positional arguments in $argv. Here’s a simple example:
...
function mybetterfunction
    ...
    \# (this is also available as \_flag\_s because of the short version)
    if set \-ql \_flag\_second
        echo $argv\[2\]
    else
        echo $argv\[1\]
        echo $argv\[3\]
...
The options will be _removed_ from $argv, so $argv\[2\] is the second _positional_ argument now:

\> mybetterfunction first \-s second third
second
COPY

For more information on argparse, like how to handle option arguments, see [the argparse documentation](cmds/argparse.html).

### PATH variables[¶](#path-variables "Link to this heading")

Path variables are a special kind of variable used to support colon-delimited path lists including [`PATH`](#envvar-PATH), [`CDPATH`](#envvar-CDPATH), `MANPATH`, `PYTHONPATH`, [`LANGUAGE`](#envvar-LANGUAGE) (for [localization](cmds/_.html)) etc. All variables that end in “PATH” (case-sensitive) become PATH variables by default.
...
CDPATH[¶](#envvar-CDPATH "Link to this definition")

A list of directories in which the [cd](cmds/cd.html) builtin looks for a new directory.

Locale Variables

Locale variables such as [`LANG`](#envvar-LANG), [`LC_ALL`](#envvar-LC_ALL), [`LC_MESSAGES`](#envvar-LC_MESSAGES), [`LC_NUMERIC`](#envvar-LC_NUMERIC) and [`LC_TIME`](#envvar-LC_TIME) set the language option for the shell and subprograms. See the section [Locale variables](#variables-locale) and [status language](cmds/status.html#status-language) for more information.

Color variables

A number of variable starting with the prefixes `fish_color` and `fish_pager_color`. See [Variables for changing highlighting colors](interactive.html#variables-color) for more information.

fish\_term24bit[¶](#envvar-fish_term24bit "Link to this definition")

If this is set to 0, fish will not output 24-bit RGB true-color sequences but the nearest color on the 256 color palette (or the 16 color palette, if [`fish_term256`](#envvar-fish_term256) is 0). See also [set\_color](cmds/set_color.html). The default is 1 but for historical reasons, fish defaults to behaving as if it was 0 on some terminals that are known to not support true-color sequences.

fish\_term256[¶](#envvar-fish_term256 "Link to this definition")

If this is set to 0 and [`fish_term24bit`](#envvar-fish_term24bit) is 0, translate RGB colors down to the 16 color palette. Also, if this is set to 0, [set\_color](cmds/set_color.html) commands such as `set_color ff0000 red` will prefer the named color.

fish\_ambiguous\_width[¶](#envvar-fish_ambiguous_width "Link to this definition")
...
fish\_emoji\_width[¶](#envvar-fish_emoji_width "Link to this definition")
...
fish\_autosuggestion\_enabled[¶](#envvar-fish_autosuggestion_enabled "Link to this definition")

controls if [Autosuggestions](interactive.html#autosuggestions) are enabled. Set it to 0 to disable, anything else to enable. By default they are on.

fish\_transient\_prompt[¶](#envvar-fish_transient_prompt "Link to this definition")

If this is set to 1, fish will redraw prompts with a `--final-rendering` argument before running a commandline, allowing you to change it before pushing it to the scrollback. This enables [transient prompts](prompt.html#transient-prompt).

fish\_handle\_reflow[¶](#envvar-fish_handle_reflow "Link to this definition")
...
fish\_key\_bindings[¶](#envvar-fish_key_bindings "Link to this definition")

the name of the function that sets up the keyboard shortcuts for the [command-line editor](interactive.html#editor).

fish\_escape\_delay\_ms[¶](#envvar-fish_escape_delay_ms "Link to this definition")

sets how long fish waits for another key after seeing an escape, to distinguish pressing the escape key from the start of an escape sequence. The default is 30ms. Increasing it increases the latency but allows pressing escape instead of alt for alt+character bindings. For more information, see [the chapter in the bind documentation](cmds/bind.html#cmd-bind-escape).

fish\_sequence\_key\_delay\_ms[¶](#envvar-fish_sequence_key_delay_ms "Link to this definition")

sets how long fish waits for another key after seeing a key that is part of a longer sequence, to disambiguate. For instance if you had bound `\cx\ce` to open an editor, fish would wait for this long in milliseconds to see a ctrl-e after a ctrl-x. If the time elapses, it will handle it as a ctrl-x (by default this would copy the current commandline to the clipboard). See also [Key sequences](interactive.html#interactive-key-sequences).

fish\_complete\_path[¶](#envvar-fish_complete_path "Link to this definition")
...
fish\_cursor\_selection\_mode[¶](#envvar-fish_cursor_selection_mode "Link to this definition")

controls whether the selection is inclusive or exclusive of the character under the cursor (see [Copy and Paste](interactive.html#killring)).

fish\_function\_path[¶](#envvar-fish_function_path "Link to this definition")
...
fish\_greeting[¶](#envvar-fish_greeting "Link to this definition")

the greeting message printed on startup. This is printed by a function of the same name that can be overridden for more complicated changes (see [funced](cmds/funced.html))

fish\_history[¶](#envvar-fish_history "Link to this definition")
...
fish\_trace[¶](#envvar-fish_trace "Link to this definition")

if set and not empty, will cause fish to print commands before they execute, similar to `set -x` in bash. The trace is printed to the path given by the \--debug-output option to fish or the [`FISH_DEBUG_OUTPUT`](#envvar-FISH_DEBUG_OUTPUT) variable. It goes to stderr by default. Set it to `all` to also trace execution of key bindings, event handlers as well as prompt and title functions.

FISH\_DEBUG[¶](#envvar-FISH_DEBUG "Link to this definition")

Controls which debug categories **fish** enables for output, analogous to the `--debug` option.

FISH\_DEBUG\_OUTPUT[¶](#envvar-FISH_DEBUG_OUTPUT "Link to this definition")

Specifies a file to direct debug output to.

fish\_user\_paths[¶](#envvar-fish_user_paths "Link to this definition")

a list of directories that are prepended to [`PATH`](#envvar-PATH). This can be a universal variable.

umask[¶](#envvar-umask "Link to this definition")

the current file creation mask. The preferred way to change the umask variable is through the [umask](cmds/umask.html) function. An attempt to set umask to an invalid value will always fail.

SHELL\_PROMPT\_PREFIX[¶](#envvar-SHELL_PROMPT_PREFIX "Link to this definition")
...
SHELL\_PROMPT\_SUFFIX[¶](#envvar-SHELL_PROMPT_SUFFIX "Link to this definition")
...
SHELL\_WELCOME[¶](#envvar-SHELL_WELCOME "Link to this definition")
...
BROWSER[¶](#envvar-BROWSER "Link to this definition")
...
argv\_opts[¶](#envvar-argv_opts "Link to this definition")

[argparse](cmds/argparse.html) sets this to the list of successfully parsed options, including option-arguments. This variable can be changed.

CMD\_DURATION[¶](#envvar-CMD_DURATION "Link to this definition")
...
fish\_kill\_signal[¶](#envvar-fish_kill_signal "Link to this definition")

the signal that terminated the last foreground job, or 0 if the job exited normally.

fish\_killring[¶](#envvar-fish_killring "Link to this definition")

a list of entries in fish’s [kill ring](interactive.html#killring) of cut text.

fish\_read\_limit[¶](#envvar-fish_read_limit "Link to this definition")

how many bytes fish will process with [read](cmds/read.html) or in a [command substitution](#expand-command-substitution).

fish\_pid[¶](#envvar-fish_pid "Link to this definition")

the process ID (PID) of the shell.

fish\_terminal\_color\_theme[¶](#envvar-fish_terminal_color_theme "Link to this definition")

a read-only variable; set to `light` or `dark` when the terminal uses a light or dark color theme respectively; set to `unknown` if the terminal does not [report its colors](terminal-compatibility.html#term-compat-query-background-color). Like [status terminal](cmds/status.html#status-terminal), this is only populated once the first interactive prompt is shown. This is used in an [\--on-variable event handler](#event) to update [syntax highlighting](interactive.html#syntax-highlighting) variables whenever the terminal’s color theme changes. See [here](cmds/fish_config.html#fish-config-theme-files) for how to specify `light` and `dark` variants in your theme.

history[¶](#envvar-history "Link to this definition")
...
hostname[¶](#envvar-hostname "Link to this definition")

the machine’s hostname.

IFS[¶](#envvar-IFS "Link to this definition")

the internal field separator that is used for word splitting with the [read](cmds/read.html) builtin. Setting this to the empty string will also disable line splitting in [command substitution](#expand-command-substitution). This variable can be changed.

last\_pid[¶](#envvar-last_pid "Link to this definition")

the process ID (PID) of the last background process.

PWD[¶](#envvar-PWD "Link to this definition")

the current working directory.

pipestatus[¶](#envvar-pipestatus "Link to this definition")

a list of exit statuses of all processes that made up the last executed pipe. See [exit status](#variables-status).

SHLVL[¶](#envvar-SHLVL "Link to this definition")

the level of nesting of shells. Fish increments this in interactive shells, otherwise it only passes it along.

status[¶](#envvar-status "Link to this definition")
...
status\_generation[¶](#envvar-status_generation "Link to this definition")
...
### The status variable[¶](#the-status-variable "Link to this heading")
...
The status can be negated with [not](cmds/not.html) (or `!`), which is useful in a [condition](#syntax-conditional). This turns a status of 0 into 1 and any non-zero status into 0.

There is also `$pipestatus`, which is a list of all `status` values of processes in a pipe. One difference is that [not](cmds/not.html) applies to `$status`, but not `$pipestatus`, because it loses information.
...
### Locale Variables[¶](#locale-variables "Link to this heading")
...
LANGUAGE[¶](#envvar-LANGUAGE "Link to this definition")

This is treated like [`LC_MESSAGES`](#envvar-LC_MESSAGES) except that it can hold multiple values, which allows to specify a priority list of languages for translation. It’s a [PATH variable](#variables-path), like in [GNU gettext](https://www.gnu.org/software/gettext/manual/html_node/The-LANGUAGE-variable.html).
...
See also [builtin \_ (underscore)](cmds/_.html).

LC\_ALL[¶](#envvar-LC_ALL "Link to this definition")

Overrides the [`LANG`](#envvar-LANG) and all other `LC_*` variables. Please use `LC_ALL` only as a temporary override.

LC\_MESSAGES[¶](#envvar-LC_MESSAGES "Link to this definition")

Determines the language in which messages are displayed, see [builtin \_ (underscore)](cmds/_.html).

LC\_NUMERIC[¶](#envvar-LC_NUMERIC "Link to this definition")

Sets the locale for [formatting numbers](cmds/printf.html).

LC\_TIME[¶](#envvar-LC_TIME "Link to this definition")

Determines how date and time are displayed. Used in the [history](cmds/history.html#history-show-time) builtin.

## Builtin commands[¶](#builtin-commands "Link to this heading")

Fish includes a number of commands in the shell directly. We call these “builtins”. These include:

-   Builtins that manipulate the shell state - [cd](cmds/cd.html) changes directory, [set](cmds/set.html) sets variables
    
-   Builtins for dealing with data, like [string](cmds/string.html) for strings and [math](cmds/math.html) for numbers, [count](cmds/count.html) for counting lines or arguments, [path](cmds/path.html) for dealing with path
    
-   [status](cmds/status.html) for asking about the shell’s status
    
-   [printf](cmds/printf.html) and [echo](cmds/echo.html) for creating output
    
-   [test](cmds/test.html) for checking conditions
    
-   [argparse](cmds/argparse.html) for parsing function arguments
    
-   [source](cmds/source.html) to read a script in the current shell (so changes to variables stay) and [eval](cmds/eval.html) to execute a string as script
    
-   [random](cmds/random.html) to get random numbers or pick a random element from a list
    
-   [read](cmds/read.html) for reading from a pipe or the terminal
    

For a list of all builtins, use `builtin -n`.

For a list of all builtins, functions and commands shipped with fish, see the [list of commands](commands.html). The documentation is also available by using the `--help` switch.

## Command lookup[¶](#command-lookup "Link to this heading")
...
If none of these work, fish runs the function [fish\_command\_not\_found](cmds/fish_command_not_found.html) and sets [`status`](#envvar-status) to 127.

You can use [type](cmds/type.html) to see how fish resolved something:
...
## Querying for user input[¶](#querying-for-user-input "Link to this heading")

Sometimes, you want to ask the user for input, for instance to confirm something. This can be done with the [read](cmds/read.html) builtin.

Let’s make up an example. This function will [glob](#expand-wildcard) the files in all the directories it gets as [arguments](#variables-argv), and [if](#syntax-conditional) there are [more than five](cmds/test.html) it will ask the user if it is supposed to show them, but only if it is connected to a terminal:

function show\_files
...
## Shell variable and function names[¶](#shell-variable-and-function-names "Link to this heading")
...
`~/.config/fish/config.fish` is sourced _after_ the snippets. This is so you can copy snippets and override some of their behavior.
...
## Future feature flags[¶](#future-feature-flags "Link to this heading")
...
remove-percent-self     off 4.0 %self is no longer expanded (use $fish\_pid)
...
-   `remove-percent-self` turns off the special `%self` expansion. It was introduced in 4.0. To get fish’s pid, you can use the [`fish_pid`](#envvar-fish_pid) variable.
    
-   `test-require-arg` removes [builtin test](cmds/test.html)’s one-argument form (`test "string"`. It was introduced in 4.0. To test if a string is non-empty, use `test -n "string"`. If disabled, any call to `test` that would change sends a [debug message](cmds/fish.html#debugging-fish) of category “deprecated-test”, so starting fish with `fish --debug=deprecated-test` can be used to find offending calls.
...
\> set \-U fish\_features regex-easyesc qmark-noglob
COPY
...
function my\_signal\_handler \--on-signal WINCH
...
Events can be fired with the [emit](cmds/emit.html) command, and do not have to be defined before. The names just need to match. For example:
...
Please note that event handlers only become active when a function is loaded, which means you need to otherwise [source](cmds/source.html) or execute a function instead of relying on [autoloading](#syntax-function-autoloading). One approach is to put it into your [configuration file](#configuration).

For more information on how to define new event handlers, see the documentation for the [function](cmds/function.html) command.

## Debugging fish scripts[¶](#debugging-fish-scripts "Link to this heading")

Fish includes basic built-in debugging facilities that allow you to stop execution of a script at an arbitrary point. When this happens you are presented with an interactive prompt where you can execute any fish command to inspect or change state (there are no debug commands as such). For example, you can check or change the value of any variables using [printf](cmds/printf.html) and [set](cmds/set.html). As another example, you can run [status print-stack-trace](cmds/status.html) to see how the current breakpoint was reached. To resume normal execution of the script, type [exit](cmds/exit.html) or ctrl\-d.

To start a debug session insert the [builtin command](cmds/breakpoint.html) `breakpoint` at the point in a function or script where you wish to gain control, then run the function or script. Also, the default action of the `TRAP` signal is to call this builtin, meaning a running script can be actively debugged by sending it the `TRAP` signal (`kill -s TRAP <PID>`). There is limited support for interactively setting or modifying breakpoints from this debug prompt: it is possible to insert new breakpoints in (or remove old ones from) other functions by using the `funced` function to edit the definition of a function, but it is not possible to add or remove a breakpoint from the function/script currently loaded and being executed.

Another way to debug script issues is to set the [`fish_trace`](#envvar-fish_trace) variable, e.g. `fish_trace=1 fish_prompt` to see which commands fish executes when running the [fish\_prompt](cmds/fish_prompt.html) function.

### Profiling fish scripts[¶](#profiling-fish-scripts "Link to this heading")
...
This will show the time for each command itself in the first column, the time for the command and every subcommand (like any commands inside of a [function](#syntax-function) or [command substitutions](#expand-command-substitution)) in the second and the command itself in the third, separated with tabs.
...
See [fish](cmds/fish.html) for more information.
