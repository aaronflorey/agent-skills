...
# Writing your own completions[¶](#writing-your-own-completions "Link to this heading")

To specify a completion, use the `complete` command. `complete` takes as a parameter the name of the command to specify a completion for. For example, to add a completion for the program `myprog` (or `myprog.exe` on [Cygwin/MSYS2](cmds/complete.html#completions-cygwin)), start the completion command with `complete -c myprog ...`

For a complete description of the various switches accepted by the `complete` command, see the documentation for the [complete](cmds/complete.html) builtin, or write `complete --help` inside the `fish` shell.
...
By default, option arguments are _optional_, so the candidates are only offered directly attached like that, so they aren’t given in this case:

\> myprog \-o <TAB>
COPY

Usually options _require_ a parameter, so you would give `--require-parameter` / `-r`:
...
complete \-c myprog \-f \--condition '\_\_fish\_seen\_subcommand\_from somesubcommand'
COPY
...
\# Here the condition is the \`\_\_fish\_seen\_subcommand\_from\` helper function.
...
#
complete \-c timedatectl \-n "not \_\_fish\_seen\_subcommand\_from $commands" \\
...
complete \-c timedatectl \-n "\_\_fish\_seen\_subcommand\_from set-timezone" \\
...
complete \-c timedatectl \-n "\_\_fish\_seen\_subcommand\_from set-local-rtc" \\
...
## Useful functions for writing completions[¶](#useful-functions-for-writing-completions "Link to this heading")
...
## Where to put completions[¶](#where-to-put-completions "Link to this heading")
...
If you have written new completions for a common Unix command, please consider sharing your work by submitting it via the instructions in [Further help and development](index.html#more-help).
