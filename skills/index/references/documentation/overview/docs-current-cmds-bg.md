Source: https://fishshell.com/docs/current/cmds/bg.html

# bg - send jobs to background[¶](#bg-send-jobs-to-background "Link to this heading")
...
`bg` sends [jobs](../language.html#syntax-job-control) to the background, resuming them if they are stopped.
...
A PID of the format `%n`, where n is an integer, will be interpreted as the PID of job number n. Job numbers can be seen in the output of [jobs](jobs.html).
