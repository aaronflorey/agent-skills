Source: https://fishshell.com/docs/current/cmds/functions.html

# functions - print or erase functions[¶](#functions-print-or-erase-functions "Link to this heading")
...
**\-c** or **\--copy** _OLDNAME_ _NEWNAME_

Creates a new function named _NEWNAME_, using the definition of the _OLDNAME_ function.
...
Causes the specified functions to be erased. This also means that it is prevented from autoloading in the current session. Use [funcsave](funcsave.html) to remove the saved copy.

**\-D** or **\--details**

Reports the path name where the specified function is defined or could be autoloaded, `stdin` if the function was defined interactively or on the command line or by reading standard input, **\-** if the function was created via [source](source.html), and `n/a` if the function isn’t available. (Functions created via [alias](alias.html) will return **\-**, because `alias` uses `source` internally. Copied functions will return where the function was copied.) If the **\--verbose** option is also specified then five lines are written:
...
**\-t** or **\--handlers-type** _TYPE_

Show all event handlers matching the given _TYPE_.
