Source: https://fishshell.com/docs/current/cmds/abbr.html

# abbr - manage fish abbreviations[¶](#abbr-manage-fish-abbreviations "Link to this heading")
...
abbr \--rename \[ \[\-c | \--command COMMAND\]... \] OLD\_WORD NEW\_WORD
...
Changed in version 3.6.0: Previous versions of this allowed saving abbreviations in universal variables. That’s no longer possible. Existing variables will still be imported and `abbr --erase` will also erase the variables. We recommend adding abbreviations to [config.fish](../language.html#configuration) by just adding the `abbr --add` command. When you run `abbr`, you will see output like this

\> abbr
abbr -a -- foo bar # imported from a universal variable, see \`help abbr\`
COPY

In that case you should take the part before the `#` comment and save it in [config.fish](../language.html#configuration), then you can run `abbr --erase` to remove the universal variable:

\> abbr \>> ~/.config/fish/config.fish
\> abbr \--erase (abbr \--list)
COPY

Alternatively you can keep them in a separate [configuration file](../language.html#configuration) by doing something like the following:

\> abbr \> ~/.config/fish/conf.d/myabbrs.fish
COPY

This will save all your abbreviations in “myabbrs.fish”, overwriting the whole file so it doesn’t leave any duplicates, or restore abbreviations you had erased. Of course any functions will have to be saved separately, see [funcsave](funcsave.html).

## “add” subcommand[¶](#add-subcommand "Link to this heading")
...
abbr \-a vim\_edit\_texts \--position command \--regex ".+\\.txt" \--function vim\_edit
COPY
...
## Other subcommands[¶](#other-subcommands "Link to this heading")

abbr \--rename \[ \[-c | \--command COMMAND\]... \] OLD\_NAME NEW\_NAME
COPY

Renames an abbreviation, from _OLD\_NAME_ to _NEW\_NAME_. For command-specific abbreviations, the `--command` options must be provided to disambiguate which abbreviation to rename.
...
Erases the abbreviation with the given name. For command-specific abbreviations, the `--command` options must be provided to disambiguate which abbreviation to rename.

abbr \-q or \--query \[NAME...\]
COPY

Return 0 (true) if one of the _NAME_ is an abbreviation.
